...
def test_max_intensity_projection_metadata_preservation():
    ...
    assert projected.metadata.system_info == {
...
def test_max_intensity_projection_2d_vs_metadata_return():
    ...
    np.testing.assert_array_equal(np.squeeze(projected_metadata.data), projected_2d)
    ...
    np.testing.assert_array_equal(
        np.squeeze(projected_metadata_ch.data), projected_2d_ch
    )
...
def test_focus_stack_basic():
    ...
    for z in range(nz):
        ...
        focused_region = np.exp(
            -((y - y_center) ** 2 + (x - x_center) ** 2) / (2 * 10**2)
        )
        ...
        data[0, z, :, :] = (
            focused_region * sharpness * 1000 + np.random.normal(0, 50, (ny, nx))
        ).astype(np.uint16)
        ...
        for method2 in results:
            if method1 != method2:
                ...
                correlation = np.corrcoef(
                    results[method1].flatten(), results[method2].flatten()
                )[0, 1]
...
def test_focus_stack_block_based():
    ...
    stacked_small_blocks = image.focus_stack(
        use_blocks=True, block_size=32, return_2d=True
    )
    ...
    stacked_smooth = image.focus_stack(
        use_blocks=True, smooth_transitions=True, return_2d=True
    )
    stacked_no_smooth = image.focus_stack(
        use_blocks=True, smooth_transitions=False, return_2d=True
    )
...
def test_estimate_tileset_acquisition_time_with_z_stack():
    ...
    result = estimate_tileset_acquisition_time(
        channel, grid_size=(3, 3), zparams=zparams
    )
...
def test_estimate_tileset_acquisition_time_custom_parameters():
    ...
    result = estimate_tileset_acquisition_time(
        channel, grid_size=(2, 2), autofocus_mode=AutoFocusMode.EACH_TILE
    )

    # Should reflect default constants
    assert (
        result["breakdown"]["stage_movement"]["time_per_move"]
        == DEFAULT_STAGE_MOVE_TIME
    )  # DEFAULT_STAGE_MOVE_TIME
    assert (
        result["breakdown"]["autofocus"]["time_per_operation"] == DEFAULT_AUTOFOCUS_TIME
    )  # DEFAULT_AUTOFOCUS_TIME
    assert result["autofocus_time"] == 4 * DEFAULT_AUTOFOCUS_TIME  # 4 tiles × 5s
...
def test_estimate_tileset_acquisition_time_scaling():
    ...
    assert (
        result_3x3["image_acquisition_time"] / result_2x2["image_acquisition_time"]
        == 9 / 4
    )
    assert (
        result_4x4["image_acquisition_time"] / result_2x2["image_acquisition_time"]
        == 16 / 4
    )
