...
def test_block_based_focus_selection():
    ...
    focus_stack[1, 0:64, 0:64] = np.random.randint(
        120, 140, (64, 64)
    )  # Low contrast noise
    ...
    z_selection = block_based_focus_selection(
        focus_stack, method="laplacian", block_size=64
    )
    ...
    assert np.all(top_left_selection == 0), (
        f"Top-left should be z=0, got unique values: {np.unique(top_left_selection)}"
    )

    # Top-right should be all z=1
    top_right_selection = z_selection[0:64, 64:128]
    assert np.all(top_right_selection == 1), (
        f"Top-right should be z=1, got unique values: {np.unique(top_right_selection)}"
    )

    # Bottom should be all z=2
    bottom_selection = z_selection[64:128, :]
    assert np.all(bottom_selection == 2), (
        f"Bottom should be z=2, got unique values: {np.unique(bottom_selection)}"
    )
...
def test_block_vs_pixel_focus_comparison():
    ...
    z_selection_blocks = block_based_focus_selection(
        focus_stack, method="laplacian", block_size=64
    )
...
def test_create_block_based_focus_stack():
    ...
    stacked = create_block_based_focus_stack(
        image_stack, method="laplacian", block_size=64
    )
    ...
    stacked_smooth = create_block_based_focus_stack(
        image_stack, smooth_transitions=True, sigma=1.0
    )
    stacked_no_smooth = create_block_based_focus_stack(
        image_stack, smooth_transitions=False
    )
...
def test_pixel_vs_block_focus_comparison():
    ...
    stacked_block = create_block_based_focus_stack(
        image_stack, method="laplacian", block_size=64, smooth_transitions=False
    )
