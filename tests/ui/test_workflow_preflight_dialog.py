...
def test_it_says_who_has_to_be_there_before_the_run(qapp):
    """The pre-run sentence comes from the estimate; an empty one shows no
    line at all, and each task row carries its own line as a tooltip."""
    from fibsem.applications.autolamella.ui.workflow_preflight_dialog import (
        WorkflowPreflightDialog,
    )
    ...
    tasks = [
        TaskEstimate(
            ...
            seconds=60.0,
            supervised=True,
            attendance="Needs you at the microscope while it runs: detection and milling.",
        )
    ]
    dlg = WorkflowPreflightDialog(
        _estimate(
            tasks=tasks,
            attendance_summary="This run needs you present for every task: Mill Undercut.",
        )
    )
    assert dlg.attendance_label.text().startswith("This run needs you present")
    assert not dlg.attendance_label.isHidden()
    row_tips = [
        w.toolTip()
        for w in dlg.findChildren(QLabel)
        if w.toolTip().startswith("Mill Undercut")
    ]
    assert row_tips and "detection and milling" in row_tips[0]
    dlg.deleteLater()

    quiet = WorkflowPreflightDialog(_estimate())
    assert quiet.attendance_label.isHidden()
    quiet.deleteLater()
