...
def test_a_compustage_draws_where_it_always_did(arctis):
    """Its FM is a flip rather than a place: the same call re-poses the slot and
    leaves it where it was, so nothing here has to ask about the mounting."""
    place = _slot_place(arctis)

    carried = at_device(arctis, "FM", place)

    assert carried.x == pytest.approx(place.x, abs=1e-12)
    assert carried.y == pytest.approx(place.y, abs=1e-12)
    assert carried.z == pytest.approx(place.z, abs=1e-12)


def test_a_place_that_cannot_be_carried_is_drawn_where_it_was_and_said(iflm, caplog):
    """An unsupported pose has no conversion. The overlay still draws, and the log
    says why the slot is not where the FM canvas can show it."""
    import logging

    place = _slot_place(iflm)
    place.t = -1.5  # no named orientation
    assert iflm.get_stage_orientation(place) == "NONE"

    with caplog.at_level(logging.WARNING):
        assert at_device(iflm, "FM", place) is place
