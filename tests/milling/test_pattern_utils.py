...
class TestBoundingBoxFunctions:
    ...
    def test_get_pattern_bounding_box_rectangle(self):
        ...
        image = FibsemImage.generate_blank_image(
            resolution=[512, 512], pixel_size=Point(x=1e-9, y=1e-9)
        )

        # Create rectangle pattern (20um x 10um) at center
        pattern = RectanglePattern(width=20e-6, height=10e-6, point=Point(x=0.0, y=0.0))
    ...
    def test_get_pattern_bounding_box_with_expansion(self):
        ...
        image = FibsemImage.generate_blank_image(
            resolution=(256, 256), pixel_size=Point(x=1e-9, y=1e-9)
        )
    ...
    def test_get_patterns_bounding_box_multiple_patterns(self):
        ...
        pattern1 = RectanglePattern(
            width=10e-6, height=10e-6, point=Point(x=-50e-6, y=-50e-6)
        )
        pattern2 = CirclePattern(radius=5e-6, point=Point(x=50e-6, y=50e-6))
    ...
    def test_get_patterns_bounding_box_empty_list(self):
        ...
        image = FibsemImage.generate_blank_image(
            resolution=(100, 100), pixel_size=Point(x=1e-9, y=1e-9)
        )
        patterns = []
...
class TestCoordinateConversion:
    ...
    def test_bbox_to_normalized_coords_full_image(self):
        ...
        image = FibsemImage.generate_blank_image(
            resolution=(100, 50), pixel_size=Point(x=1e-9, y=1e-9)
        )  # 100x50 image
    ...
    def test_coordinate_conversion_roundtrip(self):
        ...
        image = FibsemImage.generate_blank_image(
            resolution=(200, 150), pixel_size=Point(x=1e-9, y=1e-9)
        )
    ...
    def test_coordinate_conversion_different_image_sizes(self):
        ...
        bbox1 = (10, 10, 90, 90)
        image1 = FibsemImage.generate_blank_image(
            resolution=(100, 100), pixel_size=Point(x=1e-9, y=1e-9)
        )
        image2 = FibsemImage.generate_blank_image(
            resolution=(200, 200), pixel_size=Point(x=1e-9, y=1e-9)
        )
...
class TestPatternMask:
    ...
    def test_create_pattern_mask_rectangle(self):
        ...
        pattern = RectanglePattern(width=20e-6, height=20e-6, point=Point(x=0.0, y=0.0))

        mask = create_pattern_mask(
            pattern, image.data.shape, pixelsize=image.metadata.pixel_size.x
        )
    ...
    def test_create_pattern_mask_circle(self):
        ...
        pattern = CirclePattern(radius=10e-6, point=Point(x=0.0, y=0.0))
    ...
    def test_create_pattern_mask_exclusions_parameter(self):
        ...
        mask1 = create_pattern_mask(
            pattern,
            image.data.shape,
            pixelsize=image.metadata.pixel_size.x,
            include_exclusions=False,
        )
        mask2 = create_pattern_mask(
            pattern,
            image.data.shape,
            pixelsize=image.metadata.pixel_size.x,
    ...
    def test_create_pattern_mask_different_image_sizes(self):
        ...
        for width, height in [(50, 50), (100, 200), (256, 128)]:
            image = FibsemImage.generate_blank_image(
                resolution=(width, height), pixel_size=Point(x=1e-9, y=1e-9)
            )
            mask = create_pattern_mask(
                pattern, image.data.shape, pixelsize=image.metadata.pixel_size.x
            )
...
class TestUtilityFunctionIntegration:
    ...
    def test_mask_and_bbox_integration(self):
        ...
        pattern = RectanglePattern(
            width=50e-6, height=30e-6, point=Point(x=20e-6, y=-10e-6)
        )
