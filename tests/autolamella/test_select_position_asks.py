...
def test_the_flags_take_the_position_question_out_of_the_declaration():
    ...
    assert SelectMillingPositionTask.questions_for(on) == (STATE,)
    one = SelectMillingPositionTaskConfig(task_name=SETUP, confirm_position=False)
    assert SelectMillingPositionTask.questions_for(one) == (STATE,), (
        "the tilt still asks"
    )
    off = SelectMillingPositionTaskConfig(
        task_name=SETUP, confirm_position=False, confirm_tilt=False
    )
...
def test_headless_both_confirmations_go_on_the_record_unreviewed(microscope, tmp_path):
    ...
    tilt, position, point = lamella.proposals[SETUP]
    assert [p.kind for p in (tilt, position, point)] == [
        STATE,
        STATE,
        POINT_OF_INTEREST,
    ], "the tilt, then the position, then the point for afterwards"
