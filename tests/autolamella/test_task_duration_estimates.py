...
class _NoMoveConfig(MillFiducialTaskConfig):
    """A task whose _run does not open with a stage move.

    MillCoincidentTaskConfig used to be the in-tree example; since FIB-910 it moves
    onto the milling pose like every other mill, so the exception is spelled out
    here instead.
    """
...
def test_every_task_is_charged_its_opening_move_by_default():
    ...
    assert MillCoincidentTaskConfig().opens_with_stage_move is True
    assert _NoMoveConfig().opens_with_stage_move is False
