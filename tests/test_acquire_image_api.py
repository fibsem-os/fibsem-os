...
def test_parameter_validation(demo_microscope):
    ...
    with pytest.raises(
        ValueError, match="Must provide either image_settings.*or beam_type"
    ):
    ...
    image = microscope.acquire_image(
        image_settings=settings.image, beam_type=BeamType.ION
    )
    assert isinstance(image, FibsemImage)
    assert image is not None
...
def test_image_settings_precedence(demo_microscope):
    ...
    electron_settings = ImageSettings(
        beam_type=BeamType.ELECTRON, hfw=1e-6, resolution=(1024, 1024), dwell_time=1e-6
    )

    # Provide both image_settings (ELECTRON) and beam_type (ION)
    # image_settings should take precedence
    image = microscope.acquire_image(
        image_settings=electron_settings, beam_type=BeamType.ION
    )
...
def test_current_settings_usage(demo_microscope):
    ...
    assert (
        abs(image.metadata.image_settings.hfw - test_hfw) < 1e-9
    )  # Should be close to what we set
