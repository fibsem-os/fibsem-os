...
class TiledAcquisitionRunner:
    ...
    def _stitch(self) -> FibsemImage:
        ...
        # the stitched image gets its OWN metadata copy so updating it below
        # (hfw → total FOV, stitched resolution) does not mutate the caller's
        # input settings or the first tile's metadata.
        ...
        image.metadata.image_settings.resolution = (self._canvas.shape[0], self._canvas.shape[1])
