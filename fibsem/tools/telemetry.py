...
with tabs[1]:
    ...
    df_subsystem = df_subsystem.dropna(
        axis=0, how="all", subset=df_subsystem.columns[1:]
    )
    ...
    component_list = st.multiselect(
        "Select Components", ["ALL"] + components, default=["ALL"]
    )
    ...
    parameter_list = st.multiselect(
        "Select Parameters", ["ALL"] + parameters, default=["ALL"]
    )
    ...
    if len(df_subsystem) > 0:
        ...
        st.write(
            f"{len(df_subsystem)} rows, {len(df_subsystem.columns) - 1} columns ({subsystem})"
        )
        ...
        fig = px.line(
            df_subsystem,
            x="datetime",
            y=df_subsystem.columns[1:],
            title=f"Health Monitor Data - {subsystem}",
        )
...
with tabs[2]:
    ...
    for filter_str in ["Beam Is On", "Is Blanked"]:
        cols = ["datetime"] + [
            col for col in df_data.columns if filter_str in col
        ]  # or "Beam Is On" in col
        st.write(cols)
        ...
        df_groupby_fib = (
            df_beam.groupby(fib_columns).agg({"duration": "sum"}).reset_index()
        )
        df_groupby_sem = (
            df_beam.groupby(sem_columns).agg({"duration": "sum"}).reset_index()
        )
        ...
        fig_fib = px.pie(
            df_groupby_fib,
            values="duration",
            title=f"FIB Beam {filter_str}",
            names=fib_columns[0],
            hover_data=df_groupby_fib.columns,
        )
        fig_sem = px.pie(
            df_groupby_sem,
            values="duration",
            title=f"SEM Beam {filter_str}",
            names=sem_columns[0],
            hover_data=df_groupby_sem.columns,
        )
