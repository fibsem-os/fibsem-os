...
def run_beam_shift_alignment_test(
    scan_rotation: int = 180, hfw: float = 150e-6, beam_type: BeamType = BeamType.ION
):
    ...
    # parameters
    print(
        f"Running test alignment with scan rotation: {scan_rotation} degrees, HFW: {hfw}, Beam Type: {beam_type.name}"
    )
    ...
    alignment.multi_step_alignment_v2(
        microscope, ref_image=image1, use_autocontrast=True
    )
    ...
    fig.suptitle(
        f"Test Alignment: {beam_type.name} - Scan Rotation: {scan_rotation}deg"
    )
    ...
    ax[0].plot(
        [image1.data.shape[1] // 2, image1.data.shape[1] // 2],
        [0, image1.data.shape[0]],
        color="yellow",
        lw=1,
    )
    ax[0].plot(
        [0, image1.data.shape[1]],
        [image1.data.shape[0] // 2, image1.data.shape[0] // 2],
        color="yellow",
        lw=1,
    )
    ax[1].plot(
        [image2.data.shape[1] // 2, image2.data.shape[1] // 2],
        [0, image2.data.shape[0]],
        color="yellow",
        lw=1,
    )
    ax[1].plot(
        [0, image2.data.shape[1]],
        [image2.data.shape[0] // 2, image2.data.shape[0] // 2],
        color="yellow",
        lw=1,
    )
    ax[2].plot(
        [image3.data.shape[1] // 2, image3.data.shape[1] // 2],
        [0, image3.data.shape[0]],
        color="yellow",
        lw=1,
    )
    ax[2].plot(
        [0, image3.data.shape[1]],
        [image3.data.shape[0] // 2, image3.data.shape[0] // 2],
        color="yellow",
        lw=1,
    )
    plt.show()
