...
DEFAULT_MICROEXPANSION_PROTOCOL = (
    {
        ...
        "pattern": {
            ...
            "height": 18.0e-6,
            "depth": 1.0e-6,
            "distance": 10.0e-6,  # distance between microexpansion and lamella centre
        },
    },
)
...
def validate_protocol(protocol: dict):
    ...
    if "lamella" in protocol["milling"]:
        ...
        protocol["milling"][MILL_ROUGH_KEY] = {
            "stages": lamella_protocol[:-num_polishing_stages]
        }
        protocol["milling"][MILL_POLISHING_KEY] = {
            "stages": lamella_protocol[-num_polishing_stages:]
        }
        ...
        del protocol[
            ...
            "lamella"
        ]  # TODO: refactor the rest of the methods so we can remove this... don't remove just yet
...
def _convert_trench_protocol(pattern_config: dict) -> dict:
    ...
    pattern_config["spacing"] = pattern_config[
        "lamella_height"
    ] + 2 * pattern_config.get("offset", 0)
    pattern_config["upper_trench_height"] = pattern_config["trench_height"] / max(
        pattern_config.get("size_ratio", 1.0), 1.0
    )
    pattern_config["lower_trench_height"] = pattern_config["trench_height"] * min(
        pattern_config.get("size_ratio", 1.0), 1.0
    )
...
def convert_protocol_to_stage_config(
    pprotocol: dict, pkey: str, point: dict = None
) -> dict:
    ...
    pattern_config = {
        k: v
        for k, v in pprotocol.items()
        if k not in STRATEGY_KEYS + MILLING_KEYS + ["name", "type"]
    }
...
def validate_and_convert_protocol(ddict: Dict) -> Dict:
    ...
    protocol["milling"] = convert_old_milling_protocol_to_new_protocol(
        protocol["milling"]
    )
