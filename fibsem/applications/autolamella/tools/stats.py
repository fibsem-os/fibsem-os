...
st.set_page_config(
    page_title="AutoLamella Analytics", page_icon=":snowflake:", layout="wide"
)
...
encoding = st.sidebar.selectbox(
    "Encoding",
    ["cp1252", "utf-8"],
    help="Encoding of the log file (e.g. cp1252 for windows, utf-8 for linux)",
)
...
(df_experiment, df_history, _, df_steps, df_stage, df_det, df_click, df_milling) = (
    calculate_statistics_dataframe(EXPERIMENT_PATH, encoding=encoding)
)
...
try:
    ...
    cols[2].metric(
        label="Success Rate", value=f"{round((1 - n_failure / n_total) * 100, 2)}%"
    )
except:
    pass
...
with tab_experiment:
    ...
    df_steps.timestamp = df_steps.timestamp.dt.tz_localize("UTC").dt.tz_convert(
        "Australia/Sydney"
    )
    ...
    if len(df_click) > 0:
        ...
        # convert timestamp to australian timezone
        df_click.timestamp = df_click.timestamp.dt.tz_localize("UTC").dt.tz_convert(
            "Australia/Sydney"
        )
    ...
    df_lamella = df_experiment[
        ["petname", "current_stage", "failure", "failure_note", "failure_timestamp"]
    ].copy()
    # rename petname to lamella
    df_lamella.rename(columns={"petname": "lamella"}, inplace=True)
    # convert timestamp to datetime, aus timezone
    df_lamella.failure_timestamp = pd.to_datetime(
        df_lamella.failure_timestamp, unit="s"
    )
...
with tab_history:
    ...
    fig_duration = px.bar(
        ...
        color="stage",
        barmode="group",
        hover_data=df_history.columns,
        title="Lamella Duration by Stage",
    )
    ...
    fig_duration = px.bar(
        ...
        barmode="group",
        hover_data=df_history.columns,
        title="Stage Duration by Lamella",
    )
...
with tab_telemetry:
    ...
    if len(df_stage) > 0:
        ...
        df_stage.timestamp = df_stage.timestamp.dt.tz_localize("UTC").dt.tz_convert(
            "Australia/Sydney"
        )
...
with tab_automation:
    ...
    if len(df_click) > 0:
        ...
        # add total column
        df_click_lamella["total"] = df_click_lamella.groupby(
            ["lamella", "stage", "type"]
        )["timestamp"].transform("sum")

        # drop all other columns except lamella and total, stage
        df_click_lamella = df_click_lamella[
            ["lamella", "stage", "type", "total"]
        ].drop_duplicates()
    ...
    if len(df_det) > 0:
        ...
        df_group = df_group.pivot(
            index="feature", columns="is_correct", values="lamella"
        )
        ...
        df_group = (
            df_det.groupby(["lamella", "feature", "is_correct"]).count().reset_index()
        )
        df_group = df_group.pivot(
            index=["lamella", "feature"], columns="is_correct", values="stage"
        )
        ...
        df_group = (
            df_det.groupby(["stage", "feature", "is_correct"]).count().reset_index()
        )
        df_group = df_group.pivot(
            index=["stage", "feature"], columns="is_correct", values="timestamp"
        )
        ...
        ].sort_values(by="lamella")

        # loop through each row and plot the image
        # select petname, stage, feature
        petname = cols[0].selectbox(
            label="Petname", options=df_det_filt["lamella"].unique()
        )
        ...
        for row in df_det_filt.iterrows():
            ...
            ax.scatter(
                ...
                color="red",
                s=100,
                label=f"{feature} (initial)",
            )
            ax.scatter(
                px_x, px_y, marker="+", color="blue", s=100, label=f"{feature} (final)"
            )
            ax.legend()
...
with tab_workflow:
    ...
    # plot duration
    fig_duration = px.bar(
        df_history[df_history["stage"] == stage].sort_values(by="start"),
        x="petname",
        y="duration",
        color="petname",
        hover_data=df_history.columns,
    ...
    EB_IMAGE_PATHS = sorted(
        glob.glob(
            os.path.join(EXPERIMENT_PATH, f"**/*{stage}_final_high_res_eb.tif"),
            recursive=True,
        )
    )
    IB_IMAGE_PATHS = sorted(
        glob.glob(
            os.path.join(EXPERIMENT_PATH, f"**/*{stage}_final_high_res_ib.tif"),
            recursive=True,
        )
    )
    ...
    if IB_IMAGE_PATHS and EB_IMAGE_PATHS:
        ...
        for i, (petname, fname_eb, fname_ib) in enumerate(
            zip(petnames, EB_IMAGE_PATHS, IB_IMAGE_PATHS)
        ):
            eb_image = FibsemImage.load(fname_eb)
            ib_image = FibsemImage.load(fname_ib)
            cols[0].image(
                eb_image.data, caption=f"{petname} - {os.path.basename(fname_eb)}"
            )
            cols[1].image(
                ib_image.data, caption=f"{petname} - {os.path.basename(fname_ib)}"
            )
    ...
    if OVERVIEW_IMAGE and SHOW_OVERVIEW:
        ...
        key2 = cols[0].selectbox(
            label="Stage Position", options=df_history["stage"].unique()
        )
                ...
                for state in lamella.history:
                    if state.stage.name in positions.keys():
                        ...
                        if lamella.petname not in _names:
                            positions[state.stage.name].append(
                                state.microscope_state.stage_position
                            )
...
with tab_lamella:
    ...
    IMAGE_PATHS = sorted(
        glob.glob(os.path.join(EXPERIMENT_PATH, f"{lamella}/**.tif"), recursive=True)
    )
    ...
    image = FibsemImage.load(
        glob.glob(
            os.path.join(EXPERIMENT_PATH, f"{lamella}/**{IMAGE_FILENAME}"),
            recursive=True,
        )[0]
    )
    cols[1].image(
        image.data, caption=os.path.basename(IMAGE_FILENAME), use_column_width=True
    )
    ...
    # plot duration
    fig_duration = px.bar(
        df_history[df_history["petname"] == lamella].sort_values(by="start"),
        x="stage",
        y="duration",
        color="stage",
        hover_data=df_history.columns,
    ...
    fig_steps = px.bar(
        df_steps[df_steps["lamella"] == lamella].sort_values(by="timestamp"),
        x="stage",
        y="duration",
        color="step",
        hover_data=df_steps.columns,
    ...
    if not df_milling.empty:
        ...
        df_mill_filt = df_mill_filt[
            ["lamella", "stage", "step", "name", "duration", "milling_current"]
        ]
