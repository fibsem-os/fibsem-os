...
def create_pattern_mask(pattern: "BasePattern", image_shape: Tuple[int, int], pixelsize: float, include_exclusions: bool = False) -> np.ndarray:
    ...
    Args:
        pattern: BasePattern to create mask for.
    ...
    """
    pattern_mask = np.zeros(image_shape, dtype=bool)
    ...
    try:
        ...
        logging.debug(f"Failed to create mask for pattern {pattern.name}: {e}")
...
def get_pattern_bounding_box(pattern: "BasePattern", image: FibsemImage, expand_percent: float = 0.0) -> Tuple[int, int, int, int]:
    ...
    Args:
        pattern: BasePattern to get bounding box for.
...
def get_patterns_bounding_box(patterns: List["BasePattern"], image: FibsemImage, expand_percent: float = 0.0) -> Tuple[int, int, int, int]:
    ...
    Args:
        patterns: List of BasePattern to get combined bounding box for.
        image: FibsemImage for coordinate conversion.
        expand_percent: Percentage to expand bounding box by (e.g., 10.0 for 10%).
