...
def create_crosshair_shape(
    point: Point, size: int, scale: Optional[Tuple[float, float]] = None
) -> List[np.ndarray]:
    ...
    horizontal_line = np.array([[point.y, point.x - size], [point.y, point.x + size]])

    # Create vertical line at point
    vertical_line = np.array([[point.y - size, point.x], [point.y + size, point.x]])
