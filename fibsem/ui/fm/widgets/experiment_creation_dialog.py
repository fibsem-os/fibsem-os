...
class ExperimentCreationDialog(QDialog):
    ...
    def initUI(self):
        ...
        experiment_label = QLabel("Experiment File:")
        experiment_layout.addWidget(experiment_label)

        self.experiment_file_edit = QLineEdit()
        self.experiment_file_edit.setReadOnly(True)
        self.experiment_file_edit.setPlaceholderText("Select experiment.yaml file...")
        experiment_layout.addWidget(self.experiment_file_edit)

        self.browse_experiment_button = QToolButton()
        self.browse_experiment_button.setText("...")
        self.browse_experiment_button.clicked.connect(self.browse_experiment_file)
        experiment_layout.addWidget(self.browse_experiment_button)
        load_layout.addLayout(experiment_layout)
    ...
    def browse_experiment_file(self):
        ...
        if file_path:
            ...
            self.experiment_file_edit.setText(file_path)
            self.update_experiment_info()
            self.validate_input()

    def update_experiment_info(self):
        """Update experiment info display when a experiment file is selected."""
        if not self.experiment_file_path:
            self.experiment_info_label.setText("")
            return

        try:
            # Get experiment directory from experiment file path
            experiment_dir = os.path.dirname(self.experiment_file_path)

            # Try to load and parse the experiment file
            with open(self.experiment_file_path, 'r') as f:
                experiment_data = yaml.safe_load(f)

            num_positions = len(experiment_data.get('positions', []))
            created_date = experiment_data.get('created_at', 'Unknown')
    ...
    def validate_input(self):
        ...
        if self.radio_create.isChecked():
            ...
            is_valid = bool(self.experiment_file_path) and os.path.exists(self.experiment_file_path)
    ...
    def get_experiment_info(self) -> dict:
        ...
        if self.radio_create.isChecked():
            ...
            experiment_dir = os.path.dirname(self.experiment_file_path) if self.experiment_file_path else ""
            return {
                'mode': 'load',
                'experiment_file': self.experiment_file_path,
