...
class OverviewParametersWidget(QWidget):
    ...
    def set_grid_size(self, rows: int, cols: int):
        ...
        self.spinBox_rows.setValue(rows)
        self.spinBox_cols.setValue(cols)
        self.rows = rows
        self.cols = cols
        self._update_total_area_display()
    ...
    def set_overlap(self, overlap: float):
        ...
        self.doubleSpinBox_overlap.setValue(overlap)
        self.overlap = overlap
        self._update_total_area_display()

    def set_use_zstack(self, use_zstack: bool):
        """Set whether to use z-stack acquisition."""
        self.checkBox_use_zstack.setChecked(use_zstack)
        self.use_zstack = use_zstack
        self._update_zstack_planes_visibility()

    def set_autofocus_mode(self, mode: AutoFocusMode):
        """Set the auto-focus mode."""
        for i in range(self.comboBox_autofocus_mode.count()):
            if self.comboBox_autofocus_mode.itemData(i) == mode:
                self.comboBox_autofocus_mode.setCurrentIndex(i)
                self.autofocus_mode = mode
                break

    def set_autofocus_channel_name(self, channel_name: str):
        """Set the auto-focus channel name."""
        if channel_name:
            index = self.comboBox_autofocus_channel.findText(channel_name)
            if index >= 0:
                self.comboBox_autofocus_channel.setCurrentIndex(index)
                self.autofocus_channel_name = channel_name

    @property
    def overview_parameters(self) -> OverviewParameters:
        """Get current overview parameters as OverviewParameters object."""
    ...
    def overview_parameters(self, parameters: OverviewParameters):
        """Set overview parameters from OverviewParameters object."""
        self.set_grid_size(parameters.rows, parameters.cols)
        self.set_overlap(parameters.overlap)
        self.set_use_zstack(parameters.use_zstack)
        self.set_autofocus_mode(parameters.autofocus_mode)
