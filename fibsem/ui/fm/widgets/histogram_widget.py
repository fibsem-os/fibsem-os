...
class HistogramWidget(QWidget):
    ...
    def _setup_axes_for_blitting(self, data_range):
        ...
        self.histogram_patches = self.ax.bar(
            bin_centers,
            np.zeros(self.n_bins),
            width=bin_width,
            alpha=0.8,
            color="#0f7aad",
            edgecolor=f"{WHITE_ICON_COLOR}",
            animated=True,
        )

        # Create mean line artist
        self.mean_line_artist = self.ax.axvline(
            0, color="#ff6600", linestyle="--", alpha=0.9, animated=True
        )
    ...
    def _update_histogram_internal(
        self, image_data: Optional[np.ndarray], layer_name: str = ""
    ):
        ...
        try:
            ...
            range_changed = (
                self.last_data_range is None
                or abs(float(self.last_data_range[0]) - float(data_range[0]))
                > abs(float(data_range[0])) * 0.1
                or abs(float(self.last_data_range[1]) - float(data_range[1]))
                > abs(float(data_range[1])) * 0.1
            )
