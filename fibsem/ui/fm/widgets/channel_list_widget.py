...
from fibsem.fm.config import (
    fm_session_state,
    load_recent_channels,
    remove_recent_channel,
)
...
class ChannelListWidget(QWidget):
    ...
    def _on_add_channel(self) -> None:
        recents = load_recent_channels(fm_session_state(self.fm))
        if not recents:
            self.add_channel()
            return
    ...
    def _on_recent_removed(
        ...
        remove_recent_channel(channel, fm_session_state(self.fm, writable=True))
