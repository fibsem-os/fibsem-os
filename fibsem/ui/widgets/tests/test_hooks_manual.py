    ...
    def __init__(self):
        ...
        self.manager.register(
            LoggingHook(
                ...
                events=list(
                    HookEvent
                ),  # from the enum, so a new event is logged for free
            )
        )
        self.manager.register(
            NotificationHook(
        ...
        self.manager.register(
            FunctionHook(
                ...
                callback=lambda ctx: print(
                    f"[FunctionHook] {ctx.event} — {ctx.task_name} / {ctx.item_name}"
                ),
            )
        )
        ...
        layout.addWidget(
            self._section(
                "task_type filter (IMAGING hook only fires for IMAGING tasks)"
            )
        )
