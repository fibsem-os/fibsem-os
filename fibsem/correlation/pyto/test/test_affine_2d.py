...
class TestAffine2D(np_test.TestCase):
    ...
    def setUp(self):
        ...
        # basic
        self.x0 = numpy.array([[1, 0.0], [0, 1], [-1, 0]])
        self.y0_0 = 2 * self.x0
        self.y0_90 = 2 * numpy.array([[0, 1], [-1, 0], [0, -1]])
        self.y0_180 = 2 * numpy.array([[-1, 0.0], [0, -1], [1, 0]])
        self.y0_270 = 2 * numpy.array([[0, -1], [1, 0], [0, 1]])

        # parallelogram, rotation, scale, exact
        self.d1 = [-1, 2]
        self.x1 = numpy.array([[0.0, 0], [2, 0], [2, 1], [0, 1]])
        self.y1 = numpy.array([[0.0, 0], [4, 2], [3, 4], [-1, 2]]) + self.d1
        self.y1m = numpy.array([[0.0, 0], [-4, 2], [-3, 4], [1, 2]]) + self.d1

        # parallelogram, rotation, scale, not exact
        self.d2 = [-1, 2]
        self.x2 = numpy.array([[0.1, -0.2], [2.2, 0.1], [1.9, 0.8], [0.2, 1.1]])
        self.y2 = numpy.array([[0.0, 0], [4, 2], [3, 4], [-1, 2]]) + self.d2
        self.y2m = numpy.array([[0.0, 0], [-4, 2], [-3, 4], [1, 2]]) + self.d2
        ...
        self.af2_gl = numpy.array([[2.09463865, -0.84056372], [1.00406239, 1.87170871]])
    ...
    def testFindGL(self):
        ...
        np_test.assert_almost_equal(self.af1.s, self.af1_scale * numpy.diag([1, 1]))
        ...
        desired_gl = numpy.array([[2.09463865, -0.84056372], [1.00406239, 1.87170871]])
        ...
        qpsm = numpy.dot(numpy.dot(aff2d.q, aff2d.p), numpy.dot(aff2d.s, aff2d.m))
        ...
        y2_calc_gl = aff2d.transform(
            self.x2.transpose(), gl=aff2d_xy.gl, d=aff2d_xy.d, xy_axes="dim_point"
        )
        qpsm = numpy.dot(
            numpy.dot(aff2d_xy.q, aff2d_xy.p), numpy.dot(aff2d_xy.s, aff2d_xy.m)
        )
        y2_calc_qpsm = numpy.dot(qpsm, self.x2.transpose()) + numpy.expand_dims(
            aff2d_xy.d, 1
        )
        np_test.assert_almost_equal(y2_calc_gl, y2_calc_qpsm)
        # np_test.assert_almost_equal(y2_calc_gl, self.y2)
        ...
        qpsm = numpy.dot(numpy.dot(aff2d.q, aff2d.p), numpy.dot(aff2d.s, aff2d.m))
        np_test.assert_almost_equal(qpsm, aff2d.gl)
        y2m_calc_qpsm = numpy.dot(qpsm, self.x2.transpose()) + numpy.expand_dims(
            aff2d.d, 1
        )
        np_test.assert_almost_equal(y2m_calc_gl, y2m_calc_qpsm)
        # np_test.assert_almost_equal(y2_calc_gl, self.y2m)
    ...
    def testFindRS(self):
        ...
        np_test.assert_almost_equal(aff2d.scale, desired_scale)
        np_test.assert_almost_equal(aff2d.parity, -1)
        np_test.assert_almost_equal(aff2d.error, numpy.zeros_like(self.y1.transpose()))

        # test doing transformation
        y1_calc = aff2d.transform(self.x1.transpose())
        np_test.assert_almost_equal(y1_calc, self.y1m.transpose())
        qpsm = numpy.dot(numpy.dot(aff2d.q, aff2d.p), numpy.dot(aff2d.s, aff2d.m))
        y_new = numpy.dot(qpsm, self.x1.transpose()) + numpy.expand_dims(aff2d.d, 1)
        ...
        qpsm = numpy.dot(numpy.dot(af2mrs.q, af2mrs.p), numpy.dot(af2mrs.s, af2mrs.m))
    ...
    def testCompose(self):
        ...
        np_test.assert_almost_equal(af11.scale, self.af1_scale * self.af1_scale)
        np_test.assert_almost_equal(af11.parity, 1)
        np_test.assert_almost_equal(af11.rmsErrorEst, numpy.sqrt(2) * self.af1.error)
        ...
        ## After:
        np_test.assert_almost_equal(-af11m.phi, self.af1_phi + self.af1m_phi)
        np_test.assert_almost_equal(af11m.scale, self.af1_scale * self.af1m_scale)
        np_test.assert_almost_equal(af11m.parity, self.af1_parity * self.af1m_parity)
        np_test.assert_almost_equal(af11m.rmsErrorEst, numpy.sqrt(2) * self.af1.error)

        # test rms error
        af12 = Affine2D.compose(self.af1, self.af2)
        self.af1.decompose(order="qpsm")
        np_test.assert_almost_equal(
            af12.rmsErrorEst, self.af1.scale[0] * self.af2.rmsError
        )
