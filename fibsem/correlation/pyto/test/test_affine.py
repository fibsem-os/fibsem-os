...
class TestAffine(np_test.TestCase):
    ...
    def testFindTranslation(self):
        ...
        af = Affine.findTranslation(
            x=numpy.array([[1, 2, 3], [2, 3, 4]]), y=numpy.array([[2, 4, 6], [3, 6, 9]])
        )

        np_test.assert_almost_equal(af.translation, [1.0, 2.5, 4.0])
    ...
    def testFindTwoStep(self):
        ...
        # parallelogram, rotation, scale, exact
        af = Affine.findTwoStep(
            x=self.x1[0:1], y=self.y1[0:1], x_gl=self.x1, y_gl=self.y1 + 3
        )
        ...
        af = Affine.findTwoStep(
            x=self.x1[0:2], y=self.y1m[0:2], x_gl=self.x1, y_gl=self.y1m + [2, -3]
        )
        ...
        af = Affine.findTwoStep(
            x=self.x2, y=self.y2m, x_gl=self.x2, y_gl=self.y2m + [2, -3]
        )
        af_desired = Affine.find(x=self.x2, y=self.y2m)
        np_test.assert_almost_equal(af.gl, af_desired.gl)
        np_test.assert_almost_equal(af.d, af_desired.d)
        np_test.assert_almost_equal(af.rmsErrorEst, af_desired.rmsError, decimal=0)
    ...
    def checkQRDecompose(self, af):
        ...
        # s
        np_test.assert_equal((af.s > 0) * 1.0, numpy.identity(size))
    ...
    def testInverse(self):
        ...
        np_test.assert_almost_equal(numpy.dot(af.gl, af_inverse.gl), numpy.identity(2))
        ...
        np_test.assert_almost_equal(self.x2, af_inverse.transform(self.y2m), decimal=0)

        # error
        np_test.assert_almost_equal(af_inverse.error, afi.error, decimal=1)
        np_test.assert_almost_equal(af_inverse.rmsError, afi.rmsError, decimal=1)
    ...
    def testRemoveMasked(self):
        ...
        data, total_mask = Affine.removeMasked(arrays=[x, y], masks=(x_mask, y_mask))
