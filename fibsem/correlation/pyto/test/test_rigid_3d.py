...
class TestRigid3D(np_test.TestCase):
    ...
    def test_find_32_constr_ck_scale_1(self):
        ...
        x_cs = np.array([[0.0, 1, 0, 0], [0, 0, 2, 0], [0, 0, 0, 3]])
        ...
        y = np.array([[0.0, 0, -2, 0], [0, 1, 0, 0], [0, 0, 0, 3]])
        r_desired = np.array([[0.0, -1, 0], [1, 0, 0], [0, 0, 1]])
        res = Rigid3D.find_32_constr_ck(
        ...
        y = np.array([[0.0, 1, 0, 0], [0, 0, 0, -3], [0, 0, 2, 0]])
        r_desired = np.array([[1.0, 0, 0], [0, 0, -1], [0, 1, 0]])
        res = Rigid3D.find_32_constr_ck(
            x=x_cs, y=y[:2, :], scale=1, cm=False, use_jac=True
        )
        np_test.assert_almost_equal(np.dot(res.gl, x_cs), y, decimal=3)
        ...
        y = np.array([[0.0, 0, 0, 3], [0, 0, 2, 0], [0, -1, 0, 0]])
        r_desired = np.array([[0.0, 0, 1], [0, 1, 0], [-1, 0, 0]])
        res = Rigid3D.find_32_constr_ck(
            x=x_cs, y=y[:2, :], scale=1, cm=False, use_jac=True
        )
        np_test.assert_almost_equal(np.dot(res.gl, x_cs), y, decimal=3)
        ...
        # low z example 1
        x = np.array(
            [[2.0, 5.3, 7.2, 0.3, 4], [4, 3.2, 6, 5.4, 1.2], [0.5, 1.2, 0.3, 0.5, 0.1]]
        )
        ...
        np_test.assert_almost_equal(
            Rigid3D.extract_euler(res.gl, mode="x"), angles, decimal=3
        )
        ...
        x = np.array(
            [[3.2, 7.8, 0.3, 4, 5], [1.3, 3.6, 5.4, 6, 3.8], [0.1, 0.5, 0.8, 0.2, 0.3]]
        )
        ...
        y = np.dot(r, x_cm)
        y[:2, :] = y[:2, :] + np.array(
            [[0.8, -0.3, 1.1, -0.9, 0.4, -0.5], [0.1, 0.9, 0.4, 0.1, -0.3, -0.4]]
        )
        res = Rigid3D.find_32_constr_ck(
    ...
    def test_find_32_constr_ck_scale_fixed(self):
        ...
        # pi/2 rotation around z axis
        y = 3 * np.array([[0.0, 0, -2, 0], [0, 1, 0, 0], [0, 0, 0, 3]])
        r_desired = np.array([[0.0, -1, 0], [1, 0, 0], [0, 0, 1]])
        res = Rigid3D.find_32_constr_ck(
        ...
        r = Rigid3D.make_r_euler(angles, mode="x")
        res = Rigid3D.find_32_constr_ck(
            x=x_cm, y=scale * np.dot(r, x_cm)[:2, :], scale=4.8, cm=False, use_jac=True
        )
        np_test.assert_almost_equal(res.optimizeResult.fun, 0, decimal=3)
        np_test.assert_almost_equal(res.y[2, :], 4.8 * np.dot(r, x_cm)[2, :], decimal=3)
        np_test.assert_almost_equal(res.q, r, decimal=3)
        np_test.assert_almost_equal(res.s_scalar, scale)
        np_test.assert_almost_equal(
            Rigid3D.extract_euler(res.q, mode="x"), angles, decimal=3
        )
    ...
    def test_find_32_constr_ck(self):
        ...
        r = Rigid3D.make_r_euler(angles, mode="x")
        res = Rigid3D.find_32_constr_ck(
            x=x_cm, y=scale * np.dot(r, x_cm)[:2, :], scale=None, cm=False, use_jac=True
        )
        np_test.assert_almost_equal(res.optimizeResult.fun, 0, decimal=3)
        np_test.assert_almost_equal(
    ...
    def test_find_32_constr_ck_multi(self):
        ...
        r = Rigid3D.make_r_euler([0.0, 6 * np.pi / 5, 0.0])
    ...
    def test_find_32(self):
        ...
        np_test.assert_almost_equal(res.y[:2, :], y[:2, :], decimal=3)
        np_test.assert_almost_equal(
            res.y[2, :] - res.y[2:].mean(), y[2, :] - y[2:].mean(), decimal=3
        )
        ...
        res = Rigid3D.find_32(
            ...
            ninit=1,
            randome=False,
            einit=[0.2, -0.4, 0.5, np.sqrt(0.55)],
            randoms=False,
        )
        np_test.assert_almost_equal(res.gl, r, decimal=3)
        np_test.assert_almost_equal(res.optimizeResult.fun, 0, decimal=3)
    ...
    def test_approx_gl2_to_ck3(self):
        ...
        # coord system-like points
        x = np.array([[0.0, 1, 0, -1], [0, 0, 2, -1], [0, 0, 0, 0]])

        # arbitrary rotation, euler
        euler = np.array([60, 40, -35.0]) * np.pi / 180
        e_euler = Rigid3D.euler_to_ck(euler)
        r = Rigid3D.make_r_euler(euler)
        y = Rigid3D().transform(x=x, q=r, s=1.0, d=0)
        ...
        e_euler = Rigid3D.euler_to_ck(euler)
        r = Rigid3D.make_r_euler(euler)
        y = Rigid3D().transform(x=x, q=r, s=35.0, d=0)
        e_res, s_res = Rigid3D().approx_gl2_to_ck3(x=x, y=y[:2, :], ret="both")
        ...
        euler_flipped = np.array([100, 80, -170.0]) * np.pi / 180
        ...
        e_res, s_res = Rigid3D.approx_gl2_to_ck3(x=x, y=y[:2, :], xy_axes="dim_point")
    ...
    def test_gl2_to_ck3(self):
        ...
        euler = np.array([70, 50, -30.0]) * np.pi / 180
    ...
    def test_make_r_euler(self):
        ...
        res = np.dot(
            Rigid3D.make_r_euler([1.0, 2, 3]), Rigid3D.make_r_euler([-3.0, -2, -1])
        )
        np_test.assert_almost_equal(res, np.identity(3))

        # arbitrary rotation and inverse
        res = np.dot(
            Rigid3D.make_r_euler([-0.5, 1.2, 2.8]),
    ...
    def test_recalculate_translation(self):
        ...
        center = np.array([2, 0, 0]).reshape(3, 1)
        y_actual = r3d.s_scalar * (
            np.dot(r3d.q, x - center) + center
        ) + r3d.recalculate_translation(rotation_center=center)
