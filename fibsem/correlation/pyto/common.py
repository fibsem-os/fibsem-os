...
def read_labels(
    ...
    if check:
        ...
        if len(nonun["many"]) > 0:
            logging.warning("The following discs are disconnected: " + str(nonun.many))
        if len(nonun["empty"]) > 0:
            logging.warning("The following discs do not exist: " + str(nonun.empty))
...
def read_single_labels(
    ...
    # find shape
    shape = find_shape(file_name=file_name, shape=shape, suggest_shape=suggest_shape)
...
def find_shape(file_name, shape=None, suggest_shape=None):
    ...
    if shape is not None:
        ...
        if (bound_io.fileFormat == "em") or (bound_io.fileFormat == "mrc"):
            ...
            # not found
            raise ValueError("Shape of file " + file_name + "was not specified.")
...
def format_file_info(name, description, ids=None, extra=""):
    ...
    if is_multi_file(file_name=name):
        ...
        try:
            ...
            file_time = "not written"
        lines = [("# " + description + ": " + name + " (" + file_time + ")" + extra)]
