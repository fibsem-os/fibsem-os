...
def draw_multi_digit_number(
    ...
    for i, digit in enumerate(digits):
        ...
        if (
            start_col >= 0
            and end_col <= img_width
            and start_row >= 0
            and end_row <= img_height
        ):
...
def test_draw_numbers():
    ...
    img3 = draw_multi_digit_number(
        123, size=(24, 18), spacing=3, thickness=2, image_shape=(64, 100)
    )
...
def test_draw_letters():
    ...
    # Centered in larger image
    img3 = draw_text(
        "FIBSEM", size=(24, 18), spacing=3, thickness=2, image_shape=(64, 150)
    )
    axes[2].imshow(img3, cmap="gray")
