...
def scale_pixel_coordinates(
    px: Point, from_image: FibsemImage, to_image: FibsemImage
) -> Point:
...
def save_ml_feature_data(
    det: DetectedFeatures, initial_features: Optional[List[Feature]] = None
):
    ...
    try:
        if (
            det.fibsem_image.metadata is None
            or det.fibsem_image.metadata.image_settings is None
        ):
        ...
        logging.warning(
            "No image metadata found for ml images, saving with timestamp instead"
        )
...
def save_feature_data_to_csv(
    det: DetectedFeatures, features: List[dict], filename: str
):
