...
def _run_evaluation(
    ...
    plot: bool = False,
    _FEATURES_TO_IGNORE: List[str] = ["ImageCentre", "LandingPost"],
    save: bool = False,
    save_path: Path = None,
):
    ...
    for i, fname in enumerate(_prog):
        ...
        for k, CKPT in enumerate(checkpoints):
            ...
            _prog.set_description(
                f"file ({i + 1}/{len(filenames)}) {os.path.basename(fname)} - checkpoint: ({k + 1}/{len(checkpoints)}) {os.path.basename(CHECKPOINT)}"
            )
        ...
        if plot or save:
            fig = detection.plot_detections(
                dets,
                titles=["Ground Truth"]
                + [
                    os.path.basename(ckpt["checkpoint"].removesuffix(".pt"))
...
def run_evaluation_v2(
    ...
    plot: bool = False,
    _FEATURES_TO_IGNORE: List[str] = ["ImageCentre"],
    save: bool = False,
    save_path: Path = None,
):
...
def plot_evaluation_data(
    ...
    df_agg = (
        df.groupby(["checkpoint", "feature"])
        .agg({"distance": ["mean", "median", "std"]})
        .reset_index()
    )
    ...
    for threshold in thresholds:
        ...
        df_group = (
            df.groupby(["checkpoint", "feature", f"under_threshold_{threshold}px"])
            .count()
            .reset_index()
        )
        # # pivot table
        df_group = df_group.pivot(
            index=["checkpoint", "feature"],
            columns=f"under_threshold_{threshold}px",
            values="distance",
        ).reset_index()
        ...
        # # calc percentage
        df_group["under_threshold"] = df_group[True] / (
            df_group[True] + df_group[False]
        )
    ...
    df_ret["checkpoint"] = pd.Categorical(
        df_ret["checkpoint"], category_orders["checkpoint"]
    )
