...
def make_probe_image(wd: float, best_wd: float, noise: float = 0.05) -> FibsemImage:
    ...
    data = np.clip(
        (base + 1) / 2 * dtype_max * 0.8
        + np.random.randn(size, size) * dtype_max * noise,
        0,
        dtype_max,
    ).astype(np.uint16)
...
def demo_single_pass():
    ...
    print(
        f"Single pass — {result.n_iterations} steps, best WD={result.working_distance * 1e3:.3f} mm"
    )
...
def demo_multi_pass():
    ...
    result = make_synthetic_result(settings)
    print(
        f"3-pass — {result.n_iterations} total steps, best WD={result.working_distance * 1e3:.3f} mm"
    )
    for it in result.iterations:
        print(
            f"  pass {it.pass_index}  wd={it.working_distance * 1e3:.3f} mm  score={it.focus_score:.1f}"
        )
