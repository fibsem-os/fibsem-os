...
Base + TFS define `state` as a property returning `Literal["Inserted", "Retracted", "Busy", "Error", "Other"]`; Odemis defines a *method* returning `"INSERTED"/"RETRACTED"/"UNKNOWN"` (`odemis.py:217`). Every consumer compares `fm.objective.state == "Inserted"`: workflow tasks (`mill_coincident.py:156`, `acquire_fluorescence.py:190`, `select_fluorescence_position.py:58`, `manager.py:152`), the movement guard (`fibsem/microscope.py:2101`), `FibsemMovementWidget.py:316`, `objective_control_widget.py:243`. On Odemis this compares a bound method to a string → always `False`: the system permanently believes the objective is retracted; coincident-milling tasks refuse to run.
